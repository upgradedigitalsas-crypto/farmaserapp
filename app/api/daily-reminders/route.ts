  ...
  try {
    ...
    const emailPromises = Object.keys(visitsByRep).map(async (repEmail) => {
      ...
      visits.forEach((v) => {
        tableRows += `
          <tr>
            <td style="padding:12px;border-bottom:1px solid #eee;font-size:12px;color:#3b82f6;font-weight:900;white-space:nowrap;">
              ${v.startTime || '--:--'} ${v.endTime ? `- ${v.endTime}` : ''}
            </td>
            <td style="padding:12px;border-bottom:1px solid #eee;font-size:12px;">
              <strong style="color:#0f172a;text-transform:uppercase;">${v.doctorName}</strong>
            </td>
            <td style="padding:12px;border-bottom:1px solid #eee;font-size:11px;color:#64748b;text-transform:uppercase;">
      ...
      let htmlContent = `
        <div style="font-family:sans-serif;max-width:700px;margin:auto;border:1px solid #eee;border-radius:20px;padding:30px;">
          <h2 style="color:#2563eb;text-align:center;font-size:22px;margin-bottom:10px;">¡Prepara tu día de mañana! 🚀</h2>
          ...
          <table style="width:100%;border-collapse:collapse;margin-bottom:30px;">
            <thead>
              <tr style="background:#f8fafc;text-align:left;">
                ...
                <th style="padding:12px;border-bottom:2px solid #e2e8f0;font-size:11px;color:#64748b;text-transform:uppercase;">Médico / Cliente</th>
                <th style="padding:12px;border-bottom:2px solid #e2e8f0;font-size:11px;color:#64748b;text-transform:uppercase;">Ubicación</th>
              </tr>
            </thead>
            <tbody>
              ${tableRows}
          ...
          <div style="text-align:center;">
            <a href="https://www.gestiondiariafarmaser.com/reports" style="display:inline-block;background:#2563eb;color:white;padding:15px 30px;text-decoration:none;border-radius:10px;font-weight:bold;font-size:12px;text-transform:uppercase;letter-spacing:1px;">IR A LA APP PARA REPORTAR</a>
          </div>
