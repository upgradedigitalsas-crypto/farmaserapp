...
export default function ReportsPage() {
  // Conectamos con la memoria global
  ...
  // 1. Cargamos la lista de visitadores (solo si es admin)
  useEffect(() => {
    if (isAdmin) {
  ...
  // 2. Cargamos las visitas del DÍA EN CURSO según el filtro
  useEffect(() => {
    const fetchTodayVisits = async () => {
      ...
      try {
        ...
        if (isAdmin && selectedRep === 'Todos') {
          // El Gerente ve TODO lo planeado para HOY
          q = query(visitsRef, where('visitDate', '==', todayStr))
        } else {
          // Filtro por visitador específico (o el propio si es visitador)
        ...
        // Solo mostramos las que aún están "Planeadas" para que las reporten
        setVisits(data.filter((v: any) => v.status === 'Planeada'))

        // Cargamos catálogo de productos
